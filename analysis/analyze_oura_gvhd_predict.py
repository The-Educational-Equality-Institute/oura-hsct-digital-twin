...
def evaluate_alerts(
    ...
    event_date_str = str(KNOWN_EVENT_DATE)  # "YYYY-MM-DD" for comparison with alert date strings

    if event_idx is not None:
        # Find first RED alert before or on event date
        pre_event_reds = [
            a for a in alerts
            if a["level"] == "RED" and a["date"] <= event_date_str
        ]
        pre_event_yellows = [
            a for a in alerts
            if a["level"] == "YELLOW" and a["date"] <= event_date_str
        ]
