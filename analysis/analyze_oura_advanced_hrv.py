...
from config import (
    ...
    NORM_RMSSD_P25, NORM_RMSSD_P75, HSCT_RMSSD_RANGE,
)
...
from _theme import (
    ...
    ACCENT_GREEN, ACCENT_PURPLE, ACCENT_CYAN, ACCENT_INDIGO, C_HRV,
)
...
def compute_frequency_domain(timestamps: list[datetime], rmssd: np.ndarray) -> dict:
    ...
    vlf_power = float(_integrate(pgram[vlf_mask], freqs_hz[vlf_mask])) if vlf_mask.any() else 0
    lf_power = float(_integrate(pgram[lf_mask], freqs_hz[lf_mask])) if lf_mask.any() else 0
    hf_power = float(_integrate(pgram[hf_mask], freqs_hz[hf_mask])) if hf_mask.any() else 0
...
def _moving_block_bootstrap(data: np.ndarray, rng: np.random.Generator, block_size: int) -> np.ndarray:
...
# ===========================================================================
def compute_mse(rmssd: np.ndarray, max_scale: int = 20, m: int = 2, r_factor: float = 0.2) -> dict:
    ...
    """
    if not HAS_ANTROPY:
        return {"scales": [], "entropies": [], "pattern": "insufficient_data",
                "interpretation": "antropy not installed - MSE cannot be computed",
                "low_scale_mean": None, "high_scale_mean": None}
    ...
    for scale in scales:
        ...
        if n_coarse < 30:  # Need minimum length
            ...
            continue
        coarsened = np.array([
            np.mean(rmssd[i * scale : (i + 1) * scale])
    ...
    return {
        ...
        "low_scale_mean": round(np.nanmean(entropies[:5]), 4) if len(valid) >= 5 else None,
        "high_scale_mean": round(np.nanmean(entropies[10:]), 4) if len(valid) >= 10 else None,
    }
...
# ===========================================================================
def compute_rqa(rmssd: np.ndarray, embedding_dim: int = 3, delay: int = 1,
                threshold_pct: float = 10.0, max_points: int = 2000) -> dict:
...
def compute_dfa(rmssd: np.ndarray) -> dict:
    ...
    """
    if not HAS_NOLDS:
        return {"alpha1": np.nan, "alpha2": np.nan, "alpha_full": np.nan,
                "alpha1_ci_95": (np.nan, np.nan), "alpha2_ci_95": (np.nan, np.nan),
    ...
    if len(rmssd) < 64:
        return {"alpha1": np.nan, "alpha2": np.nan, "note": "Insufficient data (<64 epochs)"}
    ...
    try:
        alpha1 = nolds.dfa(rmssd, nvals=range(4, min(17, len(rmssd) // 4)),
    ...
    try:
        ...
        if max_win > 17:
            alpha2 = nolds.dfa(rmssd, nvals=range(16, max_win),
                               overlap=True, order=1)
        else:
            alpha2 = np.nan
    except Exception:
...
# ===========================================================================
def compute_entropy_measures(rmssd: np.ndarray, m: int = 2, r_factor: float = 0.2) -> dict:
    ...
    """
    if not HAS_ANTROPY:
        return {"apen": None, "sampen": None, "m": m, "r": 0, "r_factor": r_factor,
                "classification": "unknown", "interpretation": "antropy not installed",
                "reference_healthy": "SampEn ~1.5-2.5", "reference_post_hsct": "Often < 1.0"}
    ...
    try:
        ...
        sampen = float(sampen_arr[-1]) if hasattr(sampen_arr, '__len__') else float(sampen_arr)
    except Exception:
        sampen = float("nan")
...
def compute_hjorth(rmssd: np.ndarray) -> dict:
    ...
    if not HAS_ANTROPY:
        return {"activity": np.nan, "mobility": np.nan, "complexity": np.nan,
                "note": "antropy not installed", "interpretation": "Hjorth parameters cannot be computed"}
...
def compute_baevsky_si(rmssd: np.ndarray) -> dict:
    ...
    bin_width = 0.5  # ms
    bins = np.arange(np.min(rmssd) - bin_width, np.max(rmssd) + bin_width * 2, bin_width)
...
def compute_cosinor(hr_timestamps: list[datetime], hr_bpm: np.ndarray) -> dict:
    ...
    """
    if len(hr_bpm) < 48:
        return {"mesor": 0, "amplitude": 0, "acrophase_hours": 0,
    ...
    try:
        ...
        return {"mesor": float(np.mean(hr_bpm)), "amplitude": 0,
                "acrophase_hours": 0, "r_squared": 0,
...
def compute_hr_complexity(hr_bpm: np.ndarray) -> dict:
    ...
    """
    if not HAS_ANTROPY:
        return {"permutation_entropy": None, "spectral_entropy": None,
    ...
    try:
        spec_ent = float(antropy.spectral_entropy(
            data, sf=1.0, method="welch", normalize=True
        ))
    except Exception:
        spec_ent = float("nan")
...
def compute_allostatic_load(conn: sqlite3.Connection) -> dict:
    ...
    if rows and rows[0][0] is not None:
        ...
        score += int(hrv_flag)
        details["hrv_rmssd"] = {"value": round(avg_rmssd, 1), "threshold": ESC_RMSSD_DEFICIENCY,
                                "exceeded": hrv_flag, "unit": "ms"}
    ...
    if rows:
        ...
        score += int(spo2_flag)
        details["spo2"] = {"value": round(avg_spo2, 2), "threshold": 95,
                           "exceeded": spo2_flag, "unit": "%"}
    ...
    if rows:
        ...
        score += int(deep_flag)
        details["deep_sleep_pct"] = {"value": round(avg_deep, 1), "threshold": 10,
                                     "exceeded": deep_flag, "unit": "%"}
    ...
    if rows:
        ...
        rem_flag = avg_rem < 15
        score += int(rem_flag)
        details["rem_sleep_pct"] = {"value": round(avg_rem, 1), "threshold": 15,
                                    "exceeded": rem_flag, "unit": "%"}
...
"""
    ...
    kpi_row = make_kpi_row(
        ...
        make_kpi_card(
            ...
            status=ent_st, decimals=4, status_label=ent_lbl,
            detail=f"Healthy: 1.5-2.5 | {entropy.get('classification', '')}",
        ),
        make_kpi_card(
            "Toichi CVI (vagal)", toichi.get("cvi", "N/A"), "",
            status=cvi_st, decimals=4, status_label=cvi_lbl,
            detail=f"Healthy: {toichi.get('cvi_healthy_ref', '~3.26')}",
        ),
        make_kpi_card(
            "Allostatic Load", f"{allostatic.get('score', 0)}/7", "",
            status=allo_st, status_label=allo_lbl,
            detail=allostatic.get("severity", ""),
        ),
        make_kpi_card(
            "Baevsky SI", baevsky.get("si_scaled", "N/A"), "",
            status=baev_st, decimals=2, status_label=baev_lbl,
            detail=f"Normal: <150 | {baevsky.get('classification', '')}",
        ),
        make_kpi_card(
            "Cosinor Amplitude", cosinor.get("amplitude", "N/A"), "bpm",
            status=cosinor_st, decimals=2, status_label=cosinor_lbl,
            detail=f"Healthy: 15-25 bpm | Acrophase: {cosinor.get('acrophase_hhmm', '?')}",
        ),
        make_kpi_card(
            "RQA Determinism", rqa.get("determinism", "N/A"), "",
            status=rqa_st, decimals=4, status_label=rqa_lbl,
            detail=f"Healthy: >0.6 | {rqa.get('classification', '')}",
        ),
        make_kpi_card(
            "Nightly HR CV", nightly.get("cv_pct", "N/A"), "%",
            status=nightly_st, decimals=2, status_label=nightly_lbl,
    ...
    body += make_section("5. Approximate Entropy (ApEn) and Sample Entropy (SampEn)", sec5_content)
    ...
    body += make_section("10. HR Complexity (Permutation and Spectral Entropy)", sec10_content)
    ...
    allo_badge_cls = "badge-critical" if allo_score >= 5 else "badge-warning" if allo_score >= 3 else "badge-ok"
    sec12_content = f"""
...
def create_frequency_domain_fig(freq_data: dict) -> go.Figure:
    ...
    if freqs and power:
        ...
        band_defs = [
            (freqs_arr < 0.002, "Lower band", "rgba(99, 102, 241, 0.12)", ACCENT_INDIGO),
            ((freqs_arr >= 0.002) & (freqs_arr < 0.0035), "Mid band", "rgba(139, 92, 246, 0.18)", C_HRV),
            (freqs_arr >= 0.0035, "Upper band", "rgba(6, 182, 212, 0.12)", ACCENT_CYAN),
        ]
        for mask, label, fill_col, line_col in band_defs:
            if mask.any():
                fig.add_trace(go.Scatter(
                    x=freqs_arr[mask].tolist(), y=power_arr[mask].tolist(),
        ...
        for f_val, label in [(0.002, "Lower / Mid"), (0.0035, "Mid / Upper")]:
            fig.add_shape(
                type="line", x0=f_val, x1=f_val, y0=0, y1=peak_power * 1.05,
                line=dict(color="rgba(255,255,255,0.2)", width=1, dash="dash"),
            )
            fig.add_annotation(
                x=f_val, y=peak_power * 1.02, text=label,
                showarrow=False, font=dict(size=9, color=TEXT_SECONDARY),
    ...
    fig.update_layout(
        ...
        height=350,
        xaxis=dict(
            gridcolor="rgba(255,255,255,0.05)", griddash="dot", zeroline=False,
...
def create_mse_fig(mse_data: dict) -> go.Figure:
    ...
    if scales:
        ...
        fig.add_trace(go.Scatter(
            ...
            name="Healthy range", showlegend=True,
            hoverinfo="skip",
        ))

        # Healthy reference center line
        fig.add_trace(go.Scatter(
            x=scales, y=ref_values, mode="lines+markers",
            marker=dict(size=6, color=ACCENT_GREEN, symbol="diamond", line=dict(width=1, color="rgba(255,255,255,0.3)")),
        ...
        for i, s in enumerate(scales):
            if i < len(entropies) and not np.isnan(entropies[i]):
                fig.add_trace(go.Scatter(
                    x=[s, s], y=[entropies[i], ref_values[i]],
    ...
    # Patient MSE - on top
    fig.add_trace(go.Scatter(
        x=scales, y=entropies, mode="lines+markers",
        marker=dict(size=8, color=C_HRV, symbol="circle",
...
def create_rqa_fig(rmssd: np.ndarray, rqa_data: dict) -> go.Figure:
    ...
    fig.add_annotation(
        ...
        bgcolor="rgba(15,17,23,0.85)", bordercolor=BORDER_SUBTLE, borderwidth=1,
        font=dict(size=10, color=TEXT_PRIMARY), borderpad=4,
    )
...
def create_dfa_fig(rmssd: np.ndarray, dfa_data: dict) -> go.Figure:
    ...
    n_values = np.unique(np.logspace(np.log10(4), np.log10(min(N // 4, 200)), 30).astype(int))
    F_n = []
    ...
    fig.add_shape(
        type="rect", x0=float(log_n.min()), x1=log_16, y0=y_min, y1=y_max,
        fillcolor="rgba(239,68,68,0.05)", line=dict(width=0),
        layer="below",
    )
    # Alpha-2 region shade
    fig.add_shape(
        type="rect", x0=log_16, x1=float(log_n.max()), y0=y_min, y1=y_max,
    ...
    # Data points
    fig.add_trace(go.Scatter(
        x=log_n, y=log_F, mode="markers",
        marker=dict(size=8, color=C_HRV, line=dict(width=1, color="rgba(255,255,255,0.2)")),
        name="F(n) points",
    ...
    if mask1.sum() >= 3:
        ...
        fig.add_trace(go.Scatter(
            x=log_n[mask1], y=fit1, mode="lines",
        ...
        fig.add_annotation(
            x=mid_x1, y=mid_y1 + 0.12,
            text=f"<b>alpha-1 = {alpha1_val}</b><br><span style='font-size:9px'>CI: [{alpha1_ci[0]}, {alpha1_ci[1]}]</span>",
            showarrow=True, arrowhead=2, arrowcolor=ACCENT_RED, arrowwidth=1,
            bgcolor="rgba(239,68,68,0.15)", bordercolor=ACCENT_RED, borderwidth=1,
            font=dict(size=11, color=TEXT_PRIMARY), borderpad=5,
        )
    ...
    if mask2.sum() >= 3:
        ...
        fig.add_trace(go.Scatter(
            x=log_n[mask2], y=fit2, mode="lines",
        ...
        fig.add_annotation(
            x=mid_x2, y=mid_y2 - 0.12,
            text=f"<b>alpha-2 = {alpha2_val}</b><br><span style='font-size:9px'>CI: [{alpha2_ci[0]}, {alpha2_ci[1]}]</span>",
            showarrow=True, arrowhead=2, arrowcolor=ACCENT_GREEN, arrowwidth=1,
            bgcolor="rgba(16,185,129,0.15)", bordercolor=ACCENT_GREEN, borderwidth=1,
            font=dict(size=11, color=TEXT_PRIMARY), borderpad=5,
        )
...
def create_entropy_fig(rmssd: np.ndarray, entropy_data: dict) -> go.Figure:
    ...
    categories = ["ApEn<br>(patient)", "SampEn<br>(patient)", "SampEn<br>(healthy ref)", "SampEn<br>(post-HSCT ref)"]
    values = [
    ...
    border_colors = [
        "rgba(139,92,246,0.6)", "rgba(139,92,246,0.6)",
        "rgba(16,185,129,0.6)", "rgba(156,163,175,0.4)",
    ]
    ...
    fig.add_shape(
        type="line", x0=-0.5, x1=3.5, y0=1.5, y1=1.5,
        line=dict(color=ACCENT_GREEN, width=1, dash="dash"),
    )
    fig.add_annotation(
        x=3.5, y=1.5, text="Healthy lower bound",
...
def create_poincare_fig(rmssd: np.ndarray, toichi_data: dict) -> go.Figure:
    ...
    try:
        ...
        if d_max > d_min:
            sizes = 2 + 4 * (1 - (density - d_min) / (d_max - d_min))  # Invert: sparse = larger
        else:
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            size=sizes, color=x_n, colorscale="RdYlBu_r",
            showscale=True,
            colorbar=dict(
                title=dict(text="RMSSD (ms)", font=dict(size=11, color=TEXT_SECONDARY)),
                tickfont=dict(size=10, color=TEXT_SECONDARY),
    ...
    for glow_scale, glow_alpha, glow_width in [(1.15, 0.04, 6), (1.08, 0.08, 4), (1.03, 0.15, 3)]:
        x_glow = np.cos(angle) * (sd2 * glow_scale * np.cos(theta)) - np.sin(angle) * (sd1 * glow_scale * np.sin(theta)) + mean_val
        y_glow = np.sin(angle) * (sd2 * glow_scale * np.cos(theta)) + np.cos(angle) * (sd1 * glow_scale * np.sin(theta)) + mean_val
        fig.add_trace(go.Scatter(
            x=x_glow, y=y_glow, mode="lines",
    ...
    fig.add_annotation(
        ...
        text=(f"<b>Toichi:</b> CVI={toichi_data.get('cvi', '?')}, CSI={toichi_data.get('csi', '?')}<br>"
        ...
        bgcolor="rgba(26, 29, 39, 0.92)", bordercolor=BORDER_SUBTLE, borderwidth=1,
        font=dict(size=10, color=TEXT_PRIMARY), borderpad=6,
    )
...
def create_cosinor_fig(cosinor_data: dict, hr_timestamps: list[datetime], hr_bpm: np.ndarray) -> go.Figure:
    ...
    std_by_hour = [np.std(hourly_hr[h], ddof=1) if len(hourly_hr[h]) > 1 else 0 for h in hours_sorted]
    ...
    if t_plot and y_plot:
        ...
        fig.add_trace(go.Scatter(
            x=t_plot, y=y_plot, mode="lines", fill="tozeroy",
    ...
    fig.add_trace(go.Scatter(
        x=hours_sorted, y=avg_by_hour, mode="markers+lines",
        marker=dict(size=8, color=ACCENT_BLUE, line=dict(width=1, color="rgba(255,255,255,0.2)")),
        line=dict(color=ACCENT_BLUE, width=1.5),
        error_y=dict(type="data", array=std_by_hour, visible=True,
                     color="rgba(59,130,246,0.25)", thickness=1.5),
    ...
    # MESOR line
    fig.add_shape(
        type="line", x0=0, x1=24, y0=mesor, y1=mesor,
        line=dict(color=TEXT_SECONDARY, width=1, dash="dot"),
    )
    fig.add_annotation(
        x=0.5, y=mesor, text=f"MESOR = {mesor:.1f} bpm",
    ...
    fig.add_trace(go.Scatter(
        x=[acro], y=[mesor + amp], mode="markers",
        marker=dict(size=20, color="rgba(239,68,68,0.15)", line=dict(width=0)),
        showlegend=False, hoverinfo="skip",
    ))
    fig.add_trace(go.Scatter(
        x=[acro], y=[mesor + amp], mode="markers",
        marker=dict(size=12, color=ACCENT_RED, symbol="star",
    ...
    fig.add_annotation(
        ...
        showarrow=True, arrowhead=2, arrowcolor=ACCENT_RED, arrowwidth=1.5,
        ay=-40, ax=40,
        bgcolor="rgba(239,68,68,0.12)", bordercolor=ACCENT_RED, borderwidth=1,
        font=dict(size=11, color=TEXT_PRIMARY), borderpad=5,
    )
...
def create_nightly_fig(nightly_data: dict, sleep_periods: pd.DataFrame) -> go.Figure:
    ...
    if not sleep_periods.empty and "average_heart_rate" in sleep_periods.columns:
        ...
        fig.add_trace(go.Scatter(
            x=days, y=hrs, mode="lines", fill="tozeroy",
        ...
        # Main data trace
        fig.add_trace(go.Scatter(
            x=days, y=hrs, mode="lines+markers",
            marker=dict(size=5, color=ACCENT_BLUE, line=dict(width=0.5, color="rgba(255,255,255,0.15)")),
            line=dict(color=ACCENT_BLUE, width=1.5),
        ...
        if len(hrs) >= 5:
            ...
            fig.add_trace(go.Scatter(
                x=days, y=intercept + slope * x_num, mode="lines",
    ...
    fig.add_shape(
        type="line", x0=0, x1=1, xref="paper", y0=NOCTURNAL_HR_ELEVATED, y1=NOCTURNAL_HR_ELEVATED,
        line=dict(color=ACCENT_RED, width=1.5, dash="dashdot"),
    )
    fig.add_annotation(
        x=0.98, y=NOCTURNAL_HR_ELEVATED, xref="paper", yshift=10,
        text=f"Nocturnal concern ({NOCTURNAL_HR_ELEVATED} bpm)", showarrow=False,
        font=dict(size=9, color=ACCENT_RED), xanchor="right",
    )
    fig.add_shape(
    ...
    fig.add_annotation(
        x=0.98, y=65, xref="paper", yshift=-10,
        text="Healthy nocturnal mean (65 bpm)", showarrow=False,
        font=dict(size=9, color=ACCENT_GREEN), xanchor="right",
    )

    # Ruxolitinib start annotation
    fig.add_shape(
        type="line", x0=TREATMENT_START_STR, x1=TREATMENT_START_STR,
        y0=0, y1=1, yref="paper",
        line=dict(color=ACCENT_BLUE, width=2, dash="dash"),
    )
    fig.add_annotation(
        x=TREATMENT_START_STR, y=0.92, yref="paper",
...
def _run_analysis(conn: sqlite3.Connection) -> None:
    ...
    print(f"  -> Mean: {np.mean(rmssd_all):.1f} ms, Median: {np.median(rmssd_all):.1f} ms")
    ...
    sleep_df = pd.read_sql_query(
        "SELECT * FROM oura_sleep_periods ORDER BY day", conn
    )
    ...
    metrics["frequency_domain"] = {k: v for k, v in freq_data.items() if k not in ("freqs", "power")}
    figures["frequency_domain"] = create_frequency_domain_fig(freq_data)
    print(f"  -> VLF={freq_data['vlf_pct']}%, LF={freq_data['lf_pct']}%, HF={freq_data['hf_pct']}%, LF/HF={freq_data['lf_hf_ratio']}")
    ...
    metrics["mse"]["entropies"] = [round(e, 4) if not np.isnan(e) else None for e in mse_data["entropies"]]
    ...
    print(f"  -> DET={rqa_data['determinism']}, LAM={rqa_data['laminarity']}, RR={rqa_data['recurrence_rate']}")

    print("\n[7/12] DFA alpha-1/alpha-2...")
    dfa_data = compute_dfa(rmssd_all)
    metrics["dfa"] = dfa_data
    figures["dfa"] = create_dfa_fig(rmssd_all, dfa_data)
    print(f"  -> alpha-1={dfa_data['alpha1']} ({dfa_data['alpha1_ci_95']}), alpha-2={dfa_data['alpha2']}")
    ...
    print(f"  -> Activity={hjorth_data['activity']}, Mobility={hjorth_data['mobility']}, Complexity={hjorth_data['complexity']}")
    ...
    print(f"  -> CVI={toichi_data['cvi']}, CSI={toichi_data['csi']}, SD1={toichi_data['sd1']}, SD2={toichi_data['sd2']}")

    print("\n[12/12] HR-based analyses...")
    # Cosinor
    cosinor_data = compute_cosinor(hr_timestamps, hr_bpm)
    metrics["cosinor"] = {k: v for k, v in cosinor_data.items() if k not in ("t_plot", "y_plot")}
    figures["cosinor"] = create_cosinor_fig(cosinor_data, hr_timestamps, hr_bpm)
    print(f"  -> MESOR={cosinor_data['mesor']} bpm, Amplitude={cosinor_data['amplitude']} bpm, Acrophase={cosinor_data.get('acrophase_hhmm', '?')}")

    # HR complexity
    hr_complex = compute_hr_complexity(hr_bpm)
    metrics["hr_complexity"] = hr_complex
    print(f"  -> PermEnt={hr_complex.get('permutation_entropy', '?')}, SpectralEnt={hr_complex.get('spectral_entropy', '?')}")

    # Nightly HR variability
    nightly_data = compute_nightly_hr_variability(sleep_df)
    metrics["nightly_hr_variability"] = {k: v for k, v in nightly_data.items() if k != "nightly_values"}
    metrics["nightly_hr_variability"]["nightly_values"] = nightly_data.get("nightly_values", [])
    figures["nightly"] = create_nightly_fig(nightly_data, sleep_df)
    print(f"  -> CV={nightly_data.get('cv_pct', '?')}%, Mean={nightly_data.get('mean_hr', '?')} bpm")

    # Allostatic load
    allostatic_data = compute_allostatic_load(conn)
    metrics["allostatic_load"] = allostatic_data
    print(f"  -> Allostatic load: {allostatic_data['score']}/7 ({allostatic_data['severity']})")
