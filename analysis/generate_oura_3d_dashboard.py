...
def build_kpi_cards(outputs: dict, summary: dict) -> str:
    ...
    cards.append(_card("SpO2", spo2_str, "%",
                       f"BOS risk: {bos_level} ({bos.get('composite_score', 0):.0f}/100)",
                       "warning" if bos_level == "MODERATE" else "neutral"))
...
def build_spo2_panel(outputs: dict, spo2_df: pd.DataFrame) -> go.Figure:
    ...
    if components:
        ...
        fig.add_annotation(
            ...
            showarrow=False,
            font=dict(size=12, color=C_ALERT if level == "MODERATE" else TEXT_PRIMARY),
            row=1, col=2,
        )
