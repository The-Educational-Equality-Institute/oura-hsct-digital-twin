...
def build_narrative_summary(outputs: dict, summary: dict) -> str:
    ...
    temp_prob_text = (
        f", {temp_prob:.0f}% posterior probability" if temp_prob is not None else ""
    )
...
def _svg_sparkline(
    values: list[float], width: int = 80, height: int = 24, color: str = ACCENT_BLUE
) -> str:
...
def build_kpi_cards(outputs: dict, summary: dict) -> str:
    ...
    status = (
        "good"
        if temp_p and temp_p < 0.05
        else ("warning" if temp_p and temp_p < 0.1 else "neutral")
    )
    cards.append(
        _card(
            "Temperature",
            ...
            status,
            _svg_sparkline(temp_vals, color=C_POST),
        )
    )
    ...
    cards.append(
        _card(
            ...
            hrv_str,
            "ms",
            "vs healthy 42-49 ms | ESC/NASPE 1996; Shaffer & Ginsberg 2017",
            "critical",
    ...
    lhr_p = lhr.get("p_value")
    hr_detail = (
        f"Lowest HR {_format_p_value(lhr_p)}"
        if lhr_p is not None
        else "IST threshold: 90 bpm"
    )
    cards.append(
        _card(
            "Resting HR",
    ...
    cards.append(
        _card("SpO2", spo2_str, "%", f"BOS risk: {bos_detail}", bos_status(bos_level))
    )
    ...
    sleep_rux_p = (
        sleep_out.get("ruxolitinib_comparison", {})
        .get("efficiency", {})
        .get("mann_whitney_p")
    )
    eff_detail = (
        f"Rux {_format_p_value(sleep_rux_p)}"
        if sleep_rux_p is not None
    ...
    cards.append(
        _card(
            ...
            at_str,
            "SD",
            f"UKF state | {at_dir} | {_format_p_value(at_p)}"
            if at_p is not None
            else "UKF latent state",
    ...
    cards.append(
        _card(
            ...
            gvhd_str,
            "/100",
            f"7d avg: {gvhd_bm.get('latest_7d_avg', 0):.0f}"
            if gvhd_bm
            else "Composite biomarker",
...
def build_hero_its_chart(outputs: dict, readiness_df: pd.DataFrame) -> go.Figure:
    ...
    if temp_stream and temp_stream.get("dates"):
        ...
        if intervention_idx > 0 and intervention_idx < len(dates):
            fig.add_vrect(
                x0=dates[0],
                x1=RUXOLITINIB_START,
                fillcolor="rgba(69,137,255,0.04)",
                line_width=0,
            )
            fig.add_vrect(
                x0=RUXOLITINIB_START,
                x1=dates[-1],
        ...
        if p_val is not None:
            fig.add_annotation(
                x=dates[-1],
                y=max(actual[-3:]) if actual[-3:] else 0,
        ...
        if not readiness_df.empty:
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    marker=dict(
                        color=[
                            C_POST if str(d) >= RUXOLITINIB_START else C_PRE
...
def build_forest_plot(outputs: dict) -> go.Figure:
    ...
    fig = make_subplots(
        ...
        cols=2,
        subplot_titles=[
            "CausalImpact: Relative Effect (%)",
            "UKF: Latent State Shifts (SD)",
        ],
    ...
    if drug:
        ...
        sorted_drug = sorted(
            drug.items(), key=lambda x: abs(x[1].get("shift_sd", 0)), reverse=True
        )
        ...
        fig.update_xaxes(
            title_text="Shift (SD units)", row=1, col=2, gridcolor=BORDER_SUBTLE
        )
    else:
        fig.add_annotation(
            text="No UKF drug response data",
            x=0.75,
            y=0.5,
            xref="paper",
            yref="paper",
...
def build_anomaly_timeline(outputs: dict) -> go.Figure:
    ...
    if daily_scores:
        dates = [
            s["date"] if isinstance(s.get("date"), str) else str(s.get("date", ""))
            for s in daily_scores
        ]
        scores = [s.get("ensemble_score", 0) for s in daily_scores]

        colors = [
            C_ALERT if sc > threshold else "rgba(69,137,255,0.4)" for sc in scores
        ]
        border_colors = [
            C_ALERT if sc > threshold else "rgba(69,137,255,0.6)" for sc in scores
        ]
        ...
        fig.add_vline(
            x=ACUTE_EVENT, line_dash="dot", line_color=C_ALERT, line_width=1.5
        )
...
def build_biomarker_trends(outputs: dict) -> go.Figure:
    ...
    for i, (key, name) in enumerate(zip(names_order, display_names)):
        ...
        if daily:
            ...
            post_mean = (
                rux_info.get("post_mean") if rux_info.get("post_n", 0) > 0 else None
            )
            ...
            if pre_mean is not None:
                fig.add_hline(
                    y=pre_mean,
                    line_dash="dot",
                    line_color="rgba(69,137,255,0.4)",
                    line_width=1,
                    row=row,
                    col=col,
                )
...
def build_spo2_panel(outputs: dict, spo2_df: pd.DataFrame) -> go.Figure:
    ...
    if components:
        comp_names = [
            BOS_COMPONENT_LABELS.get(name, name) for name in components.keys()
        ]
        ...
        fig.add_trace(
            go.Bar(
                ...
                marker=dict(
                    color=[
                        C_ALERT if v > 30 else (C_POST if v > 10 else C_PRE)
        ...
        fig.add_annotation(
            ...
            font=dict(
                size=12,
                color=C_ALERT
                if str(level).upper() in {"MODERATE", "ELEVATED", "HIGH"}
                else TEXT_PRIMARY,
            ),
            row=1,
            col=2,
...
def build_hr_terrain(hr_df: pd.DataFrame, sleep_df: pd.DataFrame) -> go.Figure:
    ...
    fig = go.Figure(
        data=[
            go.Surface(
                ...
                colorbar=dict(
                    title=dict(text="HR (bpm)", font=dict(size=13, color=TEXT_PRIMARY)),
                    thickness=15,
                    len=0.6,
                ...
                lighting=dict(
                    ambient=0.65, diffuse=0.5, specular=0.15, roughness=0.6, fresnel=0.1
                ),
                contours=dict(
                    z=dict(
                        show=True,
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(
                title=dict(
                    text="Hours from bedtime", font=dict(size=14, color=TEXT_PRIMARY)
                ),
                range=[0, 10],
            ...
            yaxis=dict(
                title=dict(text="Night", font=dict(size=14, color=TEXT_PRIMARY)),
                tickvals=tick_vals,
                ticktext=tick_texts,
                backgroundcolor=BG_PRIMARY,
                gridcolor="rgba(255,255,255,0.06)",
            ...
            zaxis=dict(
                title=dict(text="HR (bpm)", font=dict(size=14, color=TEXT_PRIMARY)),
                range=[60, 110],
                backgroundcolor=BG_PRIMARY,
                gridcolor="rgba(255,255,255,0.06)",
                color=TEXT_PRIMARY,
...
def build_phase_space(sleep_df: pd.DataFrame, spo2_df: pd.DataFrame) -> go.Figure:
    ...
    plot_df["marker_size"] = (
        5 + (eff - eff.min()) / max((eff.max() - eff.min()), 1) * 15
    )
    ...
    for phase_name, base_color in [
        ("Pre-Ruxolitinib", C_PRE),
        ("Post-Ruxolitinib", C_POST),
    ]:
        ...
        fig.add_trace(
            go.Scatter3d(
                x=sub["average_heart_rate"],
                y=sub["average_hrv"],
                z=sub["spo2_plot"],
                mode="markers",
                marker=dict(
                    size=sub["marker_size"].values,
                    color=point_colors,
                ...
                text=[
                    ...
                    else f"<b>{row['day']}</b><br>HR: {row['average_heart_rate']:.0f} bpm<br>"
    ...
    fig.update_layout(
        ...
        scene=dict(
            xaxis=dict(
                title=dict(
                    text="Mean HR (bpm)", font=dict(size=14, color=TEXT_PRIMARY)
                ),
                backgroundcolor=BG_PRIMARY,
                gridcolor="rgba(255,255,255,0.06)",
                color=TEXT_PRIMARY,
            ...
            yaxis=dict(
                title=dict(
                    text="Mean HRV (ms)", font=dict(size=14, color=TEXT_PRIMARY)
                ),
                backgroundcolor=BG_PRIMARY,
                gridcolor="rgba(255,255,255,0.06)",
                color=TEXT_PRIMARY,
            ...
            zaxis=dict(
                title=dict(text="SpO2 (%)", font=dict(size=14, color=TEXT_PRIMARY)),
                backgroundcolor=BG_PRIMARY,
                gridcolor="rgba(255,255,255,0.06)",
                color=TEXT_PRIMARY,
                tickfont=dict(size=11),
...
def build_circadian_radar(hr_df: pd.DataFrame, outputs: dict) -> go.Figure:
    ...
    fig.add_trace(
        go.Scatterpolar(
            ...
            line=dict(color=C_PRE, width=2.5),
            marker=dict(
                size=6, color=C_PRE, line=dict(width=1, color="rgba(255,255,255,0.4)")
            ),
            fill="toself",
            fillcolor="rgba(69,137,255,0.12)",
    ...
    fig.add_trace(
        go.Scatterpolar(
            r=pre_upper + [pre_upper[0]],
            theta=theta_labels_closed,
            mode="lines",
            line=dict(color="rgba(69,137,255,0.3)", width=1, dash="dot"),
            showlegend=False,
            hoverinfo="skip",
        )
    )
    ...
    all_vals = [
        v
        for v in pre_r + (list(post.values) if not post.empty else [])
        if not np.isnan(v)
    ]
    ...
    fig.update_layout(
        ...
        polar=dict(
            ...
            angularaxis=dict(
                tickfont=dict(size=13, color=TEXT_PRIMARY, family=FONT_FAMILY),
                direction="clockwise",
                rotation=90,
                gridcolor="rgba(255,255,255,0.08)",
                linecolor=BORDER_SUBTLE,
...
def build_biosignal_timeline(
    ...
    def add_annotations(fig: go.Figure, row: int) -> None:
        fig.add_vrect(
            x0=date_min,
            x1=RUXOLITINIB_START,
            fillcolor="rgba(69,137,255,0.03)",
            line_width=0,
            row=row,
            col=1,
        )
    ...
    hr_rolling = (
        sleep_sorted["average_heart_rate"].rolling(7, min_periods=1, center=True).mean()
    )

    fig.add_trace(
        go.Scatter(
            x=dates_hr,
            y=sleep_sorted["average_heart_rate"],
            mode="markers",
            marker=dict(size=4, color=C_PRE, opacity=0.5),
            name="Nightly HR",
            showlegend=False,
    ...
    fig.add_trace(
        go.Scatter(
            x=dates_hr,
            y=sleep_sorted["lowest_heart_rate"],
            mode="markers",
            marker=dict(size=3, color=ACCENT_GREEN, opacity=0.4, symbol="diamond"),
            name="Lowest HR",
            showlegend=False,
    ...
    hrv_rolling = (
        sleep_sorted["average_hrv"].rolling(7, min_periods=1, center=True).mean()
    )
    fig.add_trace(
        go.Scatter(
            x=dates_hr,
    ...
    if not spo2_df.empty:
        fig.add_trace(
            go.Scatter(
                x=[str(d) for d in spo2_df["day_date"]],
                y=spo2_df["spo2_average"],
                mode="markers+lines",
    ...
    if not readiness_df.empty:
                ...
                marker=dict(
                    color=temp_sorted["temperature_deviation"].apply(
                        lambda v: (
                            "#EF4444"
                            if v > 0.5
                            else ("#3B82F6" if v < -0.5 else "#6B7280")
                        )
                    ),
                    opacity=0.7,
                ),
        ...
        fig.add_hline(
            y=0, line_color="rgba(255,255,255,0.2)", line_width=0.5, row=4, col=1
        )
    ...
    eff_rolling = (
        sleep_sorted["efficiency"].rolling(7, min_periods=1, center=True).mean()
    )
    fig.add_trace(
        go.Scatter(
            x=dates_hr,
    ...
    fig.add_annotation(
        text=(
            f'<span style="color:{C_PRE}">--- Ruxolitinib start ({RUXOLITINIB_START})</span>'
...
def compute_summary(
    ...
    return {
        ...
        "mean_temp_dev": readiness_df["temperature_deviation"].mean()
        if not readiness_df.empty
        else None,
        "pre_days": pre_days,
        "post_days": post_days,
        ...
        "pre_mean_hr": pre_sleep["average_heart_rate"].mean()
        if not pre_sleep.empty
        else None,
        "post_mean_hr": post_sleep["average_heart_rate"].mean()
        if not post_sleep.empty
        else None,
        "pre_mean_hrv": pre_sleep["average_hrv"].mean()
        if not pre_sleep.empty
        else None,
        "post_mean_hrv": post_sleep["average_hrv"].mean()
        if not post_sleep.empty
        else None,
...
def build_html(
    ...
    for name, pct in summary.get("phase_pcts", {}).items():
        ...
        phase_html += (
            f'<span style="color:{color};font-weight:600">{name}: {pct:.1f}%</span> '
        )
...
def main() -> int:
    ...
    json_metrics = {
        ...
        "summary_stats": {
            "mean_hr_bpm": round(summary["mean_hr"], 1)
            if summary.get("mean_hr")
            else None,
            "mean_hrv_ms": round(summary["mean_hrv"], 1)
            if summary.get("mean_hrv")
            else None,
            "mean_spo2_pct": round(summary["mean_spo2"], 1)
            if summary.get("mean_spo2")
            else None,
            "mean_sleep_efficiency_pct": round(summary["mean_efficiency"], 1)
            if summary.get("mean_efficiency")
            else None,
            "mean_temp_deviation_c": round(summary["mean_temp_dev"], 2)
            if summary.get("mean_temp_dev")
            else None,
            "mean_total_sleep_h": round(summary["mean_total_sleep_h"], 1)
            if summary.get("mean_total_sleep_h")
            else None,
        },
        "bos_risk": {
        ...
        "pre_vs_post_treatment": {
            ...
            "post_days": summary.get("post_days", 0),
            "pre_mean_hr": round(summary["pre_mean_hr"], 1)
            if summary.get("pre_mean_hr")
            else None,
            "post_mean_hr": round(summary["post_mean_hr"], 1)
            if summary.get("post_mean_hr")
            else None,
            "pre_mean_hrv": round(summary["pre_mean_hrv"], 1)
            if summary.get("pre_mean_hrv")
            else None,
            "post_mean_hrv": round(summary["post_mean_hrv"], 1)
            if summary.get("post_mean_hrv")
            else None,
        },
